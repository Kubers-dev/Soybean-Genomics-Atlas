...
RUNS_DIR = ROOT / "online_gwas_runs"
PORT = 8766
...
def prepare_r_code(code, phenotype_path, genotype_path, output_dir):
    ...
    # Online GWAS only:
    # Force rMVP plots to use the 20 soybean chromosomes.
    # This affects plotting labels only; GWAS calculations are unchanged.
    plot_override = r"""
# ============================================================
# ONLINE GWAS ONLY - SOYBEAN CHROMOSOME LABEL OVERRIDE
# ============================================================
# Force rectangular/circular rMVP plots to use chromosome labels
# 1 through 20. rMVP itself adds the "Chr" prefix.
# GWAS calculations are unchanged.
# ============================================================

if (requireNamespace("rMVP", quietly = TRUE)) {

  rMVP_ns <- asNamespace("rMVP")
  original_MVP_Report <- get("MVP.Report", envir = rMVP_ns)

  unlockBinding("MVP.Report", rMVP_ns)
  ...
  assign(
    ...
    function(...) {
      ...
      # Soybean has 20 chromosomes.
      # rMVP adds "Chr" to these labels internally.
      args$chr.labels <- as.character(1:20)

      do.call(original_MVP_Report, args)
    },
    envir = rMVP_ns
  )

  lockBinding("MVP.Report", rMVP_ns)
}

"""

    return plot_override + execution


class GWASHandler(BaseHTTPRequestHandler):

    def log_message(self, fmt, *args):
        print("[ONLINE-GWAS]", fmt % args)
    ...
    def do_POST(self):
        ...
        try:
            ...
            print(f"[ONLINE-GWAS] Run {run_id}")
            print(f"[ONLINE-GWAS] Phenotype: {phenotype_path}")
            print(f"[ONLINE-GWAS] Genotype:  {genotype_path}")
            print(f"[ONLINE-GWAS] Output:    {output_dir}")

            # ONLINE GWAS ONLY: install missing R packages automatically
            package_check = r"""
required <- c("rMVP", "readxl", "data.table", "bigmemory")
...
if (length(missing) > 0) {
  message(
    "ONLINE GWAS: Installing missing packages: ",
...
if (length(still_missing) > 0) {
  stop(
    "Required R packages could not be installed: ",
    paste(still_missing, collapse = ", ")
  )
}

message("ONLINE GWAS: Required R packages are available.")
"""

            package_check_r = input_dir / "ONLINE_GWAS_PACKAGE_CHECK.R"
            package_check_r.write_text(package_check, encoding="utf-8")
            ...
            if package_process.returncode != 0:
                return self.send_json(
                    {
                        "error": "Required R packages could not be installed.",
                        "stdout": package_process.stdout,
                        "stderr": package_process.stderr
                    },
                    500
                )
...
if __name__ == "__main__":
    ...
    ThreadingHTTPServer(("0.0.0.0", PORT), GWASHandler).serve_forever()
